...
type FileInfo struct {
	...
	Includes  []string                 // 元のC++ファイルに含まれる#includeディレクティブのリスト。
	Macros    []string                 // 元のC++ファイルに含まれるマクロ定義のリスト。
	Functions map[string]*FunctionInfo // 完全修飾関数名をキーとして関数情報を保持するマップ。これにより、各関数の詳細情報を迅速に取得できる。
}
